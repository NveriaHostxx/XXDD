...
client.on('ready', function(){
    ...
    var ms = 100000 ;
    var setGame = [`Neviria Road To 100 ${client.guilds.size} `,`invite Users ${client.users.size}`];
    var i = -1;
    var j = 0;
...
client.on('message', message => {
  ...
  if(!message.member.hasPermission('ADMINISTRATOR')) return      message.channel.send('**للأسف لا تمتلك صلاحية** `ADMINISTRATOR`' );
    ...
    message.channel.send(`☑ | Done ... The Broadcast Has Ben Send To All ${message.guild.members.size} Members`).then(m => m.delete(5000));
